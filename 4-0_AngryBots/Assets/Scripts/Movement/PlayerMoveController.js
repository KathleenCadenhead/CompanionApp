...
private var serverUpdateMaxTime = 50;
private var serverUpdateDelay = serverUpdateMaxTime;
...
function Update () {
	...
	// Update the server, making sure the object is the player
	if(gameObject.tag == "Player" && serverUpdateDelay < serverUpdateMaxTime) {
	    serverUpdateDelay++;
	} else if(gameObject.tag == "Player" && serverUpdateDelay == serverUpdateMaxTime) {
		serverUpdateDelay = 0;
...
function updatePos() {
   ...
   var download = new WWW("http://limitless-stream-3996.herokuapp.com/infos/pos/" + login.username + "/" + parseInt(character.position.x) + "/" + parseInt(character.position.z) + ".json", form); // xpos, ypos (technically zpos)
   yield download;
   if(download.error) {
