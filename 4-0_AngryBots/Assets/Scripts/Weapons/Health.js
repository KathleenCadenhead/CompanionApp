...
private var robsKilled = 0;
private var diedAlready = false;
...
function OnDamage (amount : float, fromDirection : Vector3) {
	...
	*/
	...
	if (health <= 0 && !diedAlready)
	{
	  // ADDED this patch because in original, robs "die" multiple times if you keep hitting the explosion
	   diedAlready = true; 
	    robsKilled++;
	    // ADDED
		if(gameObject.tag != "Player") {
		    // Update the server
			yield updateRobsKilled();
		}
...
function updateRobsKilled() {

   var form = new WWWForm();
   form.AddField("Accept:", "application/json");
   form.AddField("Content-type:", "application/json");
   form.AddField("Content-Length:", "0");
   var download = new WWW("http://limitless-stream-3996.herokuapp.com/infos/rob/" + login.username + "/" + robsKilled + ".json", form); // Robots killed
   robsKilled = 0;
   yield download;
   if(download.error) {
